...
func GetMostRaided(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for i := 0; i < len(networks); i++ {
			...
			buds, err := instance.GetBudsLostToRaids(&bind.CallOpts{})
			if err != nil {
			...
			if err != nil {
				fmt.Println("Failed to update lost buds record:", err.Error())
				c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to update lost buds record"})
				return
			}
		}

		c.JSON(http.StatusOK, gin.H{"most_staked": maxChain, "lostAmount":maxDif})
	}
